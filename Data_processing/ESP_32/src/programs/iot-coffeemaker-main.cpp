...
// wifi:
static WiFiHandler wifiHandler("IOT-Coffeemaker_ben");
// mqtt:
static MqttHandler mqttHandler("IOT-Coffeemaker_ben");
...
bool buttonCoffeeLeftFlag = false;
bool buttonCoffeeRightFlag = false;
...
void loop() {
    // Update all sensors and buttons
    adcConverter.update();
    ...
    // Set flags if the left or right coffee buttons are pressed
    if (buttonCoffeeLeft.isPressed()) {
        buttonCoffeeLeftFlag = true;  // Set the flag to true if the button is pressed
    }

    if (buttonCoffeeRight.isPressed()) {
        buttonCoffeeRightFlag = true;  // Set the flag to true if the button is pressed
    }
    ...
    if (publishWatch.getTimeSinceStart() >= PUBLISH_INTERVAL_MILLIS) {
        ...
        // Update jsonDoc with the flags and other sensor/button states
        jsonDoc["ButtonCoffeeLeft"] = buttonCoffeeLeftFlag;
        jsonDoc["ButtonCoffeeRight"] = buttonCoffeeRightFlag;
        ...
        // Reset the flags after the data has been published
        buttonCoffeeLeftFlag = false;
        buttonCoffeeRightFlag = false;
    }
}
